...
import (
	...
	"github.com/kulikov0/headless-client/quic/internal/flowcontrol"
	"github.com/kulikov0/headless-client/quic/internal/handshake"
	"github.com/kulikov0/headless-client/quic/internal/monotime"
	"github.com/kulikov0/headless-client/quic/internal/protocol"
	"github.com/kulikov0/headless-client/quic/internal/qerr"
	"github.com/kulikov0/headless-client/quic/internal/utils"
	"github.com/kulikov0/headless-client/quic/internal/utils/ringbuffer"
...
var newConnection = func(
	...
	params := &wire.TransportParameters{
		...
		// See https://github.com/kulikov0/headless-client/quic/pull/3806.
	...
	s.packer = newPacketPacker(srcConnID, s.connIDManager.Get, s.initialStream, s.handshakeStream, s.sentPacketHandler, s.retransmissionQueue, cs, s.framer, &s.receivedPacketHandler, s.datagramQueue, s.perspective, s.config.ChromeStyleInitialPackets)
...
var newClientConnection = func(
	...
	params := &wire.TransportParameters{
		...
		MaxBidiStreamNum:               protocol.StreamNum(s.config.MaxIncomingStreams),
		MaxUniStreamNum:                protocol.StreamNum(s.config.MaxIncomingUniStreams),
		MaxAckDelay:                    protocol.DefaultMaxAckDelay, // Use default so it's not sent (Chrome behavior)
		MaxUDPPayloadSize:              protocol.MaxPacketBufferSize,
		AckDelayExponent:               protocol.AckDelayExponent,
		// Use default value so active_connection_id_limit is not sent in transport parameters.
		// Chrome doesn't send this parameter, relying on the default value of 2.
...
func (c *Conn) preSetup() {
	...
	c.initialStream = newInitialCryptoStream(c.perspective == protocol.PerspectiveClient, c.config.DisableClientHelloScrambling)
	...
	c.receivedPacketHandler = *ackhandler.NewReceivedPacketHandler(c.rttStats, c.logger)
