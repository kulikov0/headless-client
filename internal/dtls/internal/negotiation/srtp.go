...
func ValidateSRTPSelection(
	...
	return SRTPDecision{
		ProtectionProfile:       selection.ProtectionProfile,
		MasterKeyIdentifier:     bytes.Clone(selection.MasterKeyIdentifier),
		PeerMasterKeyIdentifier: bytes.Clone(selection.MasterKeyIdentifier),
	}, nil
}
...
func readSRTPOffer(snapshot ClientHelloSnapshot) (extension.SRTPOffer, bool, error) {
	...
	if err := offer.UnmarshalData(raw.Data); err != nil {
		return extension.SRTPOffer{}, true, fmt.Errorf("%w: %w",
			srtpError(dtlserrors.ErrInvalidClientHello, alert.DecodeError), err)
	}
